...
REM	Crear Grilla de Pendientes para Sombreado (Hill-shaded). Definir azimuth del sol (-A)
	gmt grdgradient %CUT% -A160 -G%SHADOW% -Ne0.6
...
rem	gmt grdview %CUT% -R -J -JZ -O -K -p -Qs    -C%color% -I%SHADOW% >> %OUT%
	gmt grdview %CUT% -R -J -JZ -O -K -p -Qi300 -C%color% -I%SHADOW% >> %OUT% -N%BASE%+glightgray -Wf0.5 -BnSwEZ+b -Bxaf -Byaf -Bzaf+l"Altura (m)" 
rem	gmt grdview %CUT% -R -J -JZ -O -K -p -Qi300 -C%color% -I%SHADOW% >> %OUT% -BnSwEZ+b -Bxaf -Byaf -Bzaf+l"Altura (m)" 
...
REM	-----------------------------------------------------------------------------------------------------------
REM	1. Cambiar las variables BASE y TOP (lineas 13 y 14) y la escala vertical (PROZ en linea 21) para definir la exageración vertical del bloque.
REM	2. Cambiar la region geografica del bloque.
REM	3. Cambiar la resolucion del bloque 3D (-Qi300 en linea 81).
REM	4. Cambiar la vista de perpectiva del bloque (-p, línea 24).
REM	5. Cambiar los ejes que se dibujan bloque 3D (argumento -BnSwEZ+b). Probar sacar +b e intercambiar mayúsculas y minúsculas.
